/** Mentions légales. À faire relire avant mise en ligne. */
export function MentionsLegales() {
  return (
    <article className="legal">
      <h1>Mentions légales</h1>
      <p className="legal__maj">Dernière mise à jour : août 2026</p>

      <h2>Éditeur</h2>
      <p>bytechnum (site web) — Porto-Novo, Bénin.</p>

      <h2>Directeur de la publication</h2>
      <p>bytechnum.</p>

      <h2>Contact</h2>
      <p>
        <a href="mailto:apresbac@bytechnum.com">apresbac@bytechnum.com</a>.
      </p>

      <h2>Hébergement</h2>
      <p>
        Site et service de calcul hébergés sur Spaceship Starlight Hyperlift (Spaceship,
        spaceship.com).
      </p>

      <h2>Propriété intellectuelle et crédits</h2>
      <p>
        Données issues du guide d'orientation officiel du MESRS (République du Bénin). La
        dénomination et l'interface « Après mon bac » sont la propriété de l'éditeur.
      </p>
    </article>
  );
}
